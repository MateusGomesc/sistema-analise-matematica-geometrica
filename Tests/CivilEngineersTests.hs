...
testeMomentoInerciaProporcao :: Bool
testeMomentoInerciaProporcao =
  let base1 = 0.2
      altura1 = 0.4
      ...
      -- Inércia deve ser 16x maior (2 * 2³ = 16)
...
testeDeflexaoInfluenciaComprimento =
  let carga = 5000
      ...
      -- Deflexão deve ser 8x maior (2³ = 8)
      razao = def2 / def1
...
-- =================================
-- TESTES PARA CARGA CRÍTICA DE EULER
-- =================================
...
testeCargaCriticaBasica =
  let modulo = 200e9 -- 200 GPa (aço)
      ...
      esperada = (pi ^ 2 * 200e9 * 0.001) / 3 ^ 2 -- Pcr = π²EI/L²
...
testeCargaCriticaInfluenciaComprimento =
  let modulo = 200e9
      ...
      -- Carga crítica deve ser 1/4 (1/2² = 1/4)
      razao = carga2 / carga1
...
testeCargaCriticaInfluenciaInercia =
  let modulo = 200e9
      ...
      -- Carga crítica deve ser 4x maior
      razao = carga2 / carga1
...
testeVolumeConcretoUnidades =
  -- Teste com dimensões típicas de construção
  let viga = Paralelepipedo 8.0 0.25 0.55 -- 8m x 25cm x 55cm
      volume = volumeConcreto viga
...
testeCasoRealPilarConcreto =
  ...
  let lado = 0.40
      ...
      cargaCriticaOk = cargaCritica > carga * 3 -- fator segurança > 3
...
testeCasoRealLajeResidencial =
  -- Laje residencial: 4m x 6m x 12cm, carga distribuída
  let comprimento = 4.0
      largura = 6.0
      espessura = 0.12
      ...
      moduloConcreto = 25e9
      -- Para laje, considera faixa unitária para cálculo
      ...
      deflexaoOk = deflexao < comprimento / 250 -- deflexão < L/250
   in volumeOk && deflexaoOk

testeCasoRealVigaAco :: Bool
testeCasoRealVigaAco =
  -- Viga de aço: perfil equivalente a 30cm x 60cm, vão 12m
  let base = 0.30
      altura = 0.60
      vao = 12.0
      carga = 50000 -- 50 kN
...
executarTestesCivilEngineering = do
  ...
  putStrLn $ "Proporção: " ++ show testeMomentoInerciaProporcao
...
testesCivilEngineering =
  [ -- Momento de Inércia (10 testes)
    ...
    ("Momento Inércia Proporção", testeMomentoInerciaProporcao),
