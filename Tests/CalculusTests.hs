...
testeAvaliacaoTrigonometrica =
  let funcao = Funcao (Trigonometrica Seno 1 0) "f(x) = sin(x)"
   ...
   in abs (resultado - 1.0) < 0.001 -- sin(pi/2) = 1
...
testeIntegralLinear =
  let funcao = Funcao (Linear 2 0) "f(x) = 2x"
      integral = integralNumerica funcao 0 3 1000 -- Integral de 2x dx de 0 a 3 = x² de 0 a 3 = 9
   in abs (integral - 9.0) < 0.01

testeIntegralConstante :: Bool
testeIntegralConstante =
  let funcao = Funcao (Linear 0 5) "f(x) = 5"
      integral = integralNumerica funcao 0 2 1000 -- Integral de 5 dx de 0 a 2 = 5x de 0 a 2 = 10
   in abs (integral - 10.0) < 0.01

testeIntegralQuadratica :: Bool
testeIntegralQuadratica =
  let funcao = Funcao (Quadratica 1 0 0) "f(x) = x²"
      integral = integralNumerica funcao 0 2 1000 -- Integral de x² dx de 0 a 2 = x³/3 de 0 a 2 = 8/3
   in abs (integral - (8 / 3)) < 0.01

testeIntegralZero :: Bool
testeIntegralZero =
  let funcao = Funcao (Linear 1 0) "f(x) = x"
      integral = integralNumerica funcao 0 0 1000 -- Integral de 0 a 0 = 0
   in abs integral < 0.001

testeIntegralSimetrica :: Bool
testeIntegralSimetrica =
  let funcao = Funcao (Trigonometrica Seno 1 0) "f(x) = sin(x)"
      integral = integralNumerica funcao (-pi) pi 1000 -- Integral de sin(x) de -pi a pi = 0
   in abs integral < 0.1
...
testeMaximoSeno =
  let funcao = Funcao (Trigonometrica Seno 1 0) "f(x) = sin(x)"
   ...
   in case maximo of
        Just x -> abs (x - pi / 2) < 0.1 -- máximo em pi/2
        Nothing -> False
