...
testeBuscarProjetoExistente =
  let projeto1 = Projeto 1 "Projeto A" Civil Planejamento [] [] [] [] [] (fromGregorian 2025 1 1) Nothing 50000
      projeto2 = Projeto 2 "Projeto B" Mecanica EmDesenvolvimento [] [] [] [] [] (fromGregorian 2025 2 1) Nothing 75000
...
testeBuscarProjetoPrimeiro =
  let projetos =
        [ Projeto i ("Projeto " ++ show i) Civil Planejamento [] [] [] [] [] (fromGregorian 2025 1 i) Nothing (fromIntegral i * 1000)
          | i <- [1 .. 10]
        ]
      resultado = buscarProjeto 1 projetos
   in case resultado of
...
testeBuscarProjetoUltimo =
  let projetos =
        [ Projeto i ("Projeto " ++ show i) Civil Planejamento [] [] [] [] [] (fromGregorian 2025 1 1) Nothing (fromIntegral i * 1000)
          | i <- [1 .. 10]
        ]
...
testeBuscarProjetoTipoEspecifico =
  let projeto1 = Projeto 1 "Civil A" Civil Planejamento [] [] [] [] [] (fromGregorian 2025 1 1) Nothing 50000
      projeto2 = Projeto 2 "Mecanica B" Mecanica EmDesenvolvimento [] [] [] [] [] (fromGregorian 2025 1 1) Nothing 60000
      projetos = [projeto1, projeto2]
...
testeBuscarProjetoStatusEspecifico =
  let projeto = Projeto 100 "Projeto Especial" Eletrica Concluido [] [] [] [] [] (fromGregorian 2025 1 1) (Just (fromGregorian 2025 6 1)) 100000
      projetos = [projeto]
      resultado = buscarProjeto 100 projetos
   in case resultado of
        Just p -> statusProjeto p == Concluido
        Nothing -> False

testeBuscarProjetoOrcamento :: Bool
testeBuscarProjetoOrcamento =
  let projeto = Projeto 50 "Projeto Caro" Estrutural EmRevisao [] [] [] [] [] (fromGregorian 2025 1 1) Nothing 999999
      projetos = [projeto]
...
testeBuscarProjetoComDuplicataId =
  ...
  let projeto1 = Projeto 5 "Primeiro" Civil Planejamento [] [] [] [] [] (fromGregorian 2025 1 1) Nothing 10000
      projeto2 = Projeto 5 "Segundo" Mecanica EmDesenvolvimento [] [] [] [] [] (fromGregorian 2025 1 1) Nothing 20000
      projetos = [projeto1, projeto2]
      resultado = buscarProjeto 5 projetos
   in case resultado of
...
testeFiltrarProjetosTipo =
  let projeto1 = Projeto 1 "A" Civil Concluido [] [] [] [] [] (fromGregorian 2025 1 1) Nothing 50000
      projeto2 = Projeto 2 "B" Civil EmDesenvolvimento [] [] [] [] [] (fromGregorian 2025 2 1) Nothing 60000
      projeto3 = Projeto 3 "C" Mecanica Concluido [] [] [] [] [] (fromGregorian 2025 3 1) Nothing 70000
...
testeFiltrarProjetosStatus =
  let projeto1 = Projeto 1 "A" Civil Concluido [] [] [] [] [] (fromGregorian 2025 1 1) Nothing 50000
      ...
      projeto3 = Projeto 3 "C" Mecanica Concluido [] [] [] [] [] (fromGregorian 2025 3 1) Nothing 70000
      projetos = [projeto1, projeto2, projeto3]
      filtroStatus = filtrarProjetos (\p -> statusProjeto p == Concluido) projetos
   in length filtroStatus == 2

testeFiltrarProjetosOrcamento :: Bool
testeFiltrarProjetosOrcamento =
  let projetos =
        [ Projeto i ("Projeto " ++ show i) Civil Planejamento [] [] [] [] [] (fromGregorian 2025 1 1) Nothing (fromIntegral i * 10000)
          | i <- [1 .. 10]
        ]
...
testeFiltrarProjetosNenhumAtende =
  let projetos =
        [ Projeto i ("Projeto " ++ show i) Civil Planejamento [] [] [] [] [] (fromGregorian 2025 1 1) Nothing 10000
          | i <- [1 .. 5]
        ]
...
testeFiltrarProjetosNome =
  let projeto1 = Projeto 1 "Projeto Especial" Civil Planejamento [] [] [] [] [] (fromGregorian 2025 1 1) Nothing 50000
      projeto2 = Projeto 2 "Outro Projeto" Mecanica EmDesenvolvimento [] [] [] [] [] (fromGregorian 2025 1 1) Nothing 60000
      projeto3 = Projeto 3 "Projeto Normal" Civil Concluido [] [] [] [] [] (fromGregorian 2025 1 1) Nothing 70000
      projetos = [projeto1, projeto2, projeto3]
...
testeFiltrarProjetosComposicao =
  let projetos =
        [ Projeto i ("Projeto " ++ show i) tipo status [] [] [] [] [] (fromGregorian 2025 1 1) Nothing (fromIntegral i * 10000)
          | i <- [1 .. 10],
            let tipo = if even i then Civil else Mecanica,
            let status = if i <= 5 then Planejamento else Concluido
        ]
      -- Filtrar projetos Civil E Concluído E orçamento > 60000
      resultado =
        filtrarProjetos
          ( \p ->
              tipoProjeto p == Civil
                && statusProjeto p == Concluido
                && orcamento p > 60000
          )
          projetos
...
testeFiltrarProjetosData =
  let projeto1 = Projeto 1 "A" Civil Planejamento [] [] [] [] [] (fromGregorian 2025 1 15) Nothing 50000
      projeto2 = Projeto 2 "B" Civil Planejamento [] [] [] [] [] (fromGregorian 2025 2 15) Nothing 60000
      projeto3 = Projeto 3 "C" Civil Planejamento [] [] [] [] [] (fromGregorian 2025 3 15) Nothing 70000
      projetos = [projeto1, projeto2, projeto3]
...
testesEspecialista4 =
  [ -- QuickSort
    ...
    ("Buscar Projeto Meio", testeBuscarProjetoMeio),
    ("Buscar Projeto Tipo Específico", testeBuscarProjetoTipoEspecifico),
    ("Buscar Projeto Status Específico", testeBuscarProjetoStatusEspecifico),
    ("Buscar Projeto Orçamento", testeBuscarProjetoOrcamento),
    ("Buscar Projeto ID Duplicado", testeBuscarProjetoComDuplicataId),
    -- Inserir Ordenado
    ...
    ("Inserir Ordenado Negativo", testeInserirOrdenadoNegativo),
    ("Inserir Ordenado Todos Negativos", testeInserirOrdenadoTodosNegativos),
    ("Inserir Ordenado Decimal", testeInserirOrdenadoDecimal),
    ("Inserir Ordenado Um Elemento", testeInserirOrdenadoUmElemento),
    ("Inserir Ordenado String", testeInserirOrdenadoString),
    -- Construir Árvore
    ...
    ("Construir Árvore Negativos", testeConstruirArvoreNegativos),
    ("Construir Árvore Duplicados", testeConstruirArvoreDuplicados),
    ("Construir Árvore Decimal", testeConstruirArvoreDecimal),
    ("Construir Árvore Grande", testeConstruirArvoreGrande),
    ("Construir Árvore Caracteres", testeConstruirArvoreCaracteres),
    -- Buscar Árvore
    ...
    ("Buscar Árvore Negativos", testeBuscarArvoreNegativos),
    ("Buscar Árvore Strings", testeBuscarArvoreStrings),
    ("Buscar Árvore Duplicados", testeBuscarArvoreDuplicados),
    ("Buscar Árvore Um Elemento", testeBuscarArvoreUmElemento),
    ("Buscar Árvore Completa", testeBuscarArvoreCompleta),
    -- Filtrar Projetos
    ...
    ("Filtrar Projetos Todos Atendem", testeFiltrarProjetosTodosAtendem),
    ("Filtrar Projetos Nome", testeFiltrarProjetosNome),
    ("Filtrar Projetos Composição", testeFiltrarProjetosComposicao),
    ("Filtrar Projetos Data", testeFiltrarProjetosData)
  ]