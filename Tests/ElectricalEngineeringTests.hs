...
-- =================================
-- TESTES PARA POTÊNCIA RESISTIVA (R × I²)
-- =================================
...
-- =================================
-- TESTES PARA POTÊNCIA POR TENSÃO (V²/R)
-- =================================
...
-- =================================
-- TESTES PARA RESISTÊNCIA EM SÉRIE
-- =================================
...
-- =================================
-- TESTES PARA RESISTÊNCIA EM PARALELO
-- =================================
...
testeResistenciaParaleloValoresAltos =
  ...
  let resistencias = [1e6, 2e6]
      total = resistenciaParalelo resistencias
      esperada = (1e6 * 2e6) / (1e6 + 2e6) -- ≈ 666.67kΩ
   in abs (total - esperada) < 1000
...
testeResistenciaParaleloAssimetrico =
  ...
  let resistencias = [1, 1000]
      total = resistenciaParalelo resistencias
      -- Resultado próximo do menor resistor
...
testeImpedanciaACBasica =
  let resistencia = 3 -- 3 Ω
      ...
      esperada = 5 -- Z = sqrt(R² + X²) = sqrt(9 + 16) = 5Ω
...
testeImpedanciaACMotorCA =
  ...
  let resistencia = 2
      reatancia = 6
      impedancia = impedanciaAC resistencia reatancia
      esperada = sqrt (4 + 36) -- sqrt(40) ≈ 6.32Ω
...
testeImpedanciaACTransformador =
  ...
  let resistencia = 0.5
      reatancia = 2.5
      impedancia = impedanciaAC resistencia reatancia
      esperada = sqrt (0.25 + 6.25) -- sqrt(6.5) ≈ 2.55Ω
   in abs (impedancia - esperada) < 0.01
...
testeImpedanciaACValoresAltos =
  ...
  let resistencia = 1000
      reatancia = 1500
      impedancia = impedanciaAC resistencia reatancia
      esperada = sqrt (1000000 + 2250000) -- ≈ 1802.8Ω
...
testeImpedanciaACBaixaImpedancia =
  ...
  let resistencia = 0.1
      reatancia = 0.2
      impedancia = impedanciaAC resistencia reatancia
      esperada = sqrt (0.01 + 0.04) -- sqrt(0.05) ≈ 0.224Ω
   in abs (impedancia - esperada) < 0.001
...
-- =================================
-- TESTES PARA CONVERSÕES POLARES/RETANGULARES
-- =================================
...
testeRetangularParaPolarQuadrante2 =
  ...
  let x = -3
      ...
      (r, theta) = retangularParaPolar x y
      rEsperado = 5
      thetaEsperado = atan2 4 (-3) -- ângulo no 2° quadrante
...
testeCasoRealMotorTrifasico =
  ...
  let tensao = 380 -- 380V
      ...
      -- Conversão para forma retangular/polar
...
testeCasoRealFiltroPassaBaixa =
  ...
  let resistencia = 1000
      ...
      -- Conversão para análise fasorial
      (r, theta) = retangularParaPolar resistencia reatanciaCapacitiva
      -- Verificações (reatância capacitiva é negativa, mas usamos módulo)
