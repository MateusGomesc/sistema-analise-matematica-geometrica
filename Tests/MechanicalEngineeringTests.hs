...
testeTorqueAngulo45Graus =
  let forca = 100
      ...
      angulo = pi / 4 -- 45°
      torque = calcularTorque forca raio angulo
      esperado = 100 * 1.0 * sin (pi / 4) -- 100 * sqrt(2)/2 ≈ 70.71
...
-- =================================
-- TESTES PARA ACELERAÇÃO CENTRÍPETA
-- =================================
...
testeEnergiaPotencialHidreletrica :: Bool
testeEnergiaPotencialHidreletrica =
...
testeCentroMassaAviao =
  -- Avião: fuselagem (5000kg) em x=10m, asas (2000kg) em x=8m, cauda (500kg) em x=20
...
testeCentroMassaGrande =
  ...
  let pontos = [(fromIntegral i, fromIntegral (i * 2)) | i <- [1 .. 10]]
      centro = centroMassaX pontos
      -- Soma(i * 2i) / Soma(i) = Soma(2i²) / Soma(i) = 2*Soma(i²) / Soma(i)
      -- Para i=1..10: Soma(i) = 55, Soma(i²) = 385
...
testeCasoRealGuindaste =
  ...
  let massa = 2000 -- 2 toneladas
      ...
      forcaLanca = 150000 -- força na lança
      -- Cálculos
...
testeCasoRealTurbina =
  ...
  let raio = 50 -- raio das pás 50 m
      ...
      velocidadePonta = velocidadeVento * 3 -- ponta 3x mais rápida
      -- Cálculos
...
testeCasoRealBalanca =
  ...
  let pontos = [(5, -1), (10, 0), (15, 2)] -- massas na balança
      centro = centroMassaX pontos
      -- Para equilíbrio, momento total deve ser zero em relação ao centro
      momentoTotal = sum [(m * (x - centro)) | (m, x) <- pontos]
      -- Verificação: momento deve ser próximo de zero (equilíbrio)
...
executarTestesEngenhariaMecanica = do
  ...
  putStrLn "======================================"
  putStrLn "\n-- TESTES TORQUE --"
  putStrLn $ "Básico: " ++ show testeTorqueBasico
  ...
  putStrLn $ "Hidrelétrica: " ++ show testeEnergiaPotencialHidreletrica
...
testesEngenhariaMecanica =
  [ -- Torque (10 testes)
    ...
    ("Energia Cinética Carro", testeEnergiaCineticaCarro),
    ("Energia Cinética Bala", testeEnergiaCineticaBala),
    ...
    ("Energia Potencial Hidrelétrica", testeEnergiaPotencialHidreletrica),
